...
Route::group(['middleware' => 'authentic','prefix'=>'media','roles' => ['client','admin']],function (){
    ...
    Route::post('upload/image/{albn?}', 'Frontend\_PhotoController@store' )->name('media.image.post');
    Route::post('upload/profile', 'Frontend\_PhotoController@storeProfile' )->name('profile.image.post');
    ...
    Route::get('profile/video/{u_id?}', 'Frontend\_VideoController@getUsersVideos' )->name('profile.video.get');
    Route::get('profile/image/{u_id?}', 'Frontend\_PhotoController@getUsersImage')->name('profile.image.get');
});
