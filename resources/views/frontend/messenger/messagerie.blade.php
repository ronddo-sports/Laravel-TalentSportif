...
@section('content')
                        ...
                        <div class="dropdown all_conversation">
                            ...
                            <ul class="dropdown-menu" aria-labelledby="dropdownMenu2">
                                <li><a href="#"> All Conversation </a>  <ul class="sub_menu_ list-unstyled">
                                        ...
                                        <li><a href="#">Something else herefg</a></li>
   ...
   $uid = \Illuminate\Support\Facades\Auth::id();
    $cue = \Illuminate\Support\Facades\DB::table('message_users')
            ->where('message_users.user_id', $uid)
            ->Join('message_cue', function ($join) {
                $join->on('message_users.thread_id', '=', 'message_cue.id')
                ->where('message_cue.subject', '=', 'inbox');
            })
            ->join('messages','messages.thread_id','=','message_cue.id')
            ->select('messages.*')
   ...
   <h4>{{json_encode($cue)}}</h4>
   <h3>Sujet : Inbox</h3>
    @foreach($cue as $sms)
        <h4 style="color: {{$sms->user_id == $uid ? 'red' : 'blue'}};float: {{$sms->user_id == $uid ? 'right' : 'left'}}">
            {{ $sms->user_id }} :  {{ $sms->body }}</h4><br>
    @endforeach
